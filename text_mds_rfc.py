from SupervisedLearning import MDSPipeline
...
if __name__ == '__main__':
	...
	grid = {'rfc__criterion':['gini'], 'rfc__n_estimators':list(range(10, 120, 10)),
			...
			'MDS__n_components':list(range(2, 46, 2))}#, 'pca__n_components':list(range(43, 64, 1))}
	"""
	...
	out = Parallel(n_jobs=-2)(delayed(knn_run)(MDSPipeline([('rfc', RandomForestClassifier())], kfold=GroupKFold(n_splits=10)), params) for params in ParameterGrid(grid))
	...
	MASENm = K['MDS__n_components']
	C = grid['rfc__n_estimators']
	MASEN = grid['MDS__n_components']
	out_simp = {(k[0]['rfc__n_estimators'], k[0]['MDS__n_components']):k[1] for k in dicts}
	print(out_simp.keys())
	...
	im1, cbar1 = heatmap(Z, C, MASEN, y_label='rfc__n_estimators', x_label='MDS__n_components', ax=ax4, dataset_name='JHU', steps=MDSPipeline([('rfc', RandomForestClassifier())]).get_steps_name(),
